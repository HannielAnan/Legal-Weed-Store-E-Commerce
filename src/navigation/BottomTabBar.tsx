...
export const BottomTabBar: React.FC = () => {
  ...
  <div
    key={tab.id}
    onClick={() => {
      // Prevent "Search", "Wishlist", and "Profile" tabs from working
      if (!['Search','Order' , 'Wishlist', 'Profile'].includes(tab.name)) {
        dispatch(actions.setScreen(tab.name));
      }
    }}
  >
    <div style={{cursor: !['Search', 'Wishlist', 'Profile'].includes(tab.name) ? 'pointer' : 'default'}}>
      <tab.icon
        color={
          currentTabScreen === tab.name
            ? theme.colors.mainYellow
